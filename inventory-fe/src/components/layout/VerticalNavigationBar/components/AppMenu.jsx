...
const MenuItemLink = ({
  ...
  if (item.isDisabled || !item.url) {
    return <span className={clsx(className, 'disabled')} aria-disabled="true">
      {item.icon && <span className="nav-icon">
          <IconifyIcon icon={item.icon} />
        </span>}
      <span className="nav-text">{item.label}</span>
  ...
  return <Link to={item.url ?? ''} target={item.target} onClick={handleClick} className={clsx(className, {
...
const AppMenu = ({
  ...
  return <ul className="navbar-nav">
      ...
      return <Fragment key={item.key + idx}>
            {item.isTitle ? <li className={clsx('menu-title', item.className)}>
                <span className="menu-title-content">
                  {item.icon && <IconifyIcon icon={item.icon} className="menu-title-icon" />}
